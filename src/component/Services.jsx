     ...
     <div className='container mx-auto text-center '>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8'>
            {services.map((services, index)=>(
                ...
                key={index} className='bg-[#1c1a2b] rounded-lg p-6 text-center hover:shadow-lg hover:shadow-purple-700 transition-shadow duration-300 ' >
                    {services.icon }
                <h3 className='text-lg sm:text-xl lg:2xl: font-semibold text-purple-500'>{services.title}</h3>
                <p className='text-sm sm:text-base lg:text-lg text-gray-400'>{services.description}</p>
                </motion.div>
            ))}
        </div>
     </div>
